...
class DocumentRAGAgent(BaseAgent):
    ...
    async def _prepare(self, query: str, user_id: str, **kwargs) -> tuple[str, list[ContentChunk], Diagnostics]:
        ...
        logger.info(f"[RAG] Selected Document: {document_id}")
        logger.info(f"[RAG] Selected Collection: {collection_id}")

        intent_keywords = [
            "greeting", "help", "capabilities", "uploaded document count", 
            "document names", "indexed status", "collections", "settings", "metadata",
            "files are indexed", "document am i chatting with", "many documents are uploaded", "collections exist"
        ]
        is_metadata = any(k in query.lower() for k in intent_keywords)
        ...
        if is_metadata:
            logger.info("[RAG] Intent classified as metadata. Skipping vector search.")
        else:
        ...
        if not is_metadata and len(context_chunks) == 0:
            prompt = (
                f"The user asked about '{query}'. "
                f"However, vector retrieval returned zero chunks for the selected document/scope. "
                f"Explain that the selected document does not contain the requested information instead of searching unrelated documents."
            )
        else:
            prompt = CitationService.build_prompt_context(query, context_chunks, scope_text)
            if is_metadata:
                prompt += "\n\nAnswer the user's question directly using application metadata or simple conversational responses."

        session_id = kwargs.get("session_id")
        if session_id:
            try:
                history = await asyncio.to_thread(MemoryService.get_history, session_id, 8)
                # Ignore the most recent user message as it is the current query itself!
                history = [h for h in history if not (h['role'] == 'user' and h['content'] == query)]
                if history:
